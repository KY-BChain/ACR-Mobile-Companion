# ACR Mobile Companion v0.5 P1/P2 Generator Review

**Review ID:** `MOB-P1P2-GEN-REVIEW-002`  
**Generator:** `src/vP1P2/apply-v0.5-update.sh`  
...
1. **Zsh is appropriate.** The `#!/bin/zsh` shebang, `set -euo pipefail`, quoted heredocs and UTF-8 content all parse successfully with `/usr/bin/zsh -n`. The Mac's default login shell is not material when the file is invoked explicitly as `zsh src/vP1P2/apply-v0.5-update.sh`.
