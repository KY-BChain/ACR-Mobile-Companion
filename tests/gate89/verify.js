...
assert.equal(appConfig.expo.ios.buildNumber, '49');
...
console.log(`PASS Gates 8-9: Dynamic Type never disabled across ${screens.length} screens and ${components.length} components; accessibility affordances on every screen with announced alerts; About renders the derived v0.6.7 (Build 49) identity with no hardcoded version; ${locales.length} locales key-identical (${base.length} keys) with Arabic RTL; no placeholder or debug text in shipped screens`);
