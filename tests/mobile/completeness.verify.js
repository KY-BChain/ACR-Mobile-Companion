...
// Build 47 (v0.6.6), still pinned in Build 49: incomplete-case flow (M1–M9), signed-in-while-offline
...
const requestBuilder = compile('src/api/requestBuilder.ts', {
  '../config/appIdentity': { MOBILE_BUILD_ID: 'mob-v0.6.7+49' },
...
const script = read('scripts/build49-review-service.sh');
assert.match(script, new RegExp(`CLIENT_BUILD_ID="mob-v${app.version.replace(/\./g, '\\.')}\\+${app.ios.buildNumber}"`));
assert.match(script, /PREVIOUS_CLIENT_BUILD_IDS="\$\{ACR_PREVIOUS_CLIENT_BUILD_IDS-mob-v0\.6\.7\+48\}"/);
assert.match(script, /fixtures\/demo-\$CLIENT_BUILD_ID/);
console.log('STATIC PASS Android app ID com.acragent.companion, fail-closed release signing, arm64/armv7 split with universal APK, Build 49 service identity');
