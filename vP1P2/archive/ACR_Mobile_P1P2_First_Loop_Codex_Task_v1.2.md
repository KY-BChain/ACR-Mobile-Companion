# MOB-P1P2-FIRST-LOOP-001 — Codex Task Contract v1.2

**Status:** Approved execution candidate  
**Supersedes:** `ACR_Mobile_P1P2_First_Loop_Codex_Task_v1.1.md`  
...
**Canonical campaign directory:** `/Users/Kraken/DAPP/acr-mobile-companion/vP1P2/`  
...
- Kraken confirms that the latest pre-P1/P2 source on local `main`, already committed and pushed, is the source baseline that produced the working three-screen iOS Simulator, development-client and standalone iPhone 13 application.
- The expected remote is the private repository `KY-BChain/ACR-Mobile-Companion`; its Git identity must be verified locally by SHA before editing.
...
- run `vP1P2/apply-v0.5-update.sh` against the repository;
...
3. Approved P1/P2 field contract, Input Completeness Review and validation mock-ups in `vP1P2/`.
4. `vP1P2/ACR_Mobile_v0.5_P1P2_Application_Guide.md`.
5. `vP1P2/ACR_Mobile_P1P2_Generator_Review_v0.3.md` as the known-defect register.
6. `vP1P2/apply-v0.5-update.sh` and its embedded outputs as candidate code only.
...
3. Record current branch, full HEAD SHA and `git status --short --branch`.
4. Record `git remote -v` and the current upstream. Prove `origin` resolves to `KY-BChain/ACR-Mobile-Companion` using its HTTPS or SSH form. Do not change the remote.
5. If authorised and `origin` is correct, run `git fetch --prune origin`, then record the full `origin/main` SHA and `git rev-list --left-right --count HEAD...origin/main`.
6. The pre-feature baseline passes only when the current branch is `main` or an explicitly authorised feature branch created from synced `main`, local `HEAD` equals `origin/main`, ahead/behind against `origin/main` is `0 0`, and the working tree is clean except for explicitly accepted campaign-control files. The number “118 changes” is contextual history, not a baseline identifier.
7. If the repository is behind, ahead unexpectedly, diverged, lacks the expected upstream, or contains an unexpected overlapping change, stop `BLOCKED_GIT_BASELINE`. Do not pull, merge, stash or reset.
8. Inspect `package.json`, the lockfile, Expo/React Native and `expo-dev-client` versions, application config, Metro/Babel config, native-directory policy, Pod/workspace/scheme presence and repository scripts.
9. Determine whether `ios/` is generated-only or contains direct native modifications that Prebuild could overwrite.
10. Map navigation, state, reset, i18n, validation, Review/request, attestation and Result boundaries.
11. State explicitly whether repository evidence is consistent with Kraken's accepted build history. Do not claim to have re-proved the historical physical build merely by inspecting source.
...
2. Git root, current authorised branch, full local HEAD, full `origin/main`, canonical remote identity and `0/0` ahead/behind evidence at task start;
...
1. Git promotion is performed separately under Kraken's authority using GitHub Desktop. Codex does not infer commit/push authority from remote-read permission.
