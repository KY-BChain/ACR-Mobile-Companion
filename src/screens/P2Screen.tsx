  ...
  return (
    <ScreenLayout title={t('p2:title')} subtitle={t('assessment:stepSubtitle', { current: 5, total: 5, title: t('p2:stepTitle') })} bannerText={t('p2:banner')} steps={{ total: 5, current: 5 }} footer={<>
      ...
      <ACRCard title={t('p2:cardTitle')}>
        <Text style={styles.label}>{t('p2:ecogScore')} <Text style={styles.small}>· {t('common:optional')}</Text></Text>
        <ACRInput value={p2.ecogScore} onChangeText={(value) => setP2({ ecogScore: value })} keyboardType="numeric" hint={t('p2:ecogHint')} />
        {!ecogValid ? <Text accessibilityRole="alert" style={styles.error}>{t('p2:ecogError')}</Text> : null}
        <Text style={styles.label}>{t('p2:pdl1Status')} <Text style={styles.small}>· {t('common:optional')}</Text></Text>
        ...
        <Text style={styles.label}>{t('p2:her2Low')} <Text style={styles.small}>· {t('common:optional')}</Text></Text>
        ...
        <Text style={styles.label}>{t('p2:lvef')} <Text style={styles.small}>· {t('common:optional')}</Text></Text>
        <ACRInput value={p2.lvef} onChangeText={(value) => setP2({ lvef: value })} keyboardType="numeric" hint={t('p2:lvefHint')} />
        {!lvefValid ? <Text accessibilityRole="alert" style={styles.error}>{t('p2:lvefError')}</Text> : null}
        <Text style={styles.label}>{t('p2:treatmentIntent')} <Text style={styles.small}>· {t('common:optional')}</Text></Text>
